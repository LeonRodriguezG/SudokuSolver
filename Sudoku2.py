...
def iterativeDeepening(values):
    ...
    totalSearched = 0
    totalNumberWriten = 0
    while (not depthLimitedNonOrdered(values,limDepth)) and (bottomTouched):
        iniVars()
        limDepth+=1
    iniVars()
    return depthLimitedNonOrdered(values,limDepth)
...
def breadthFirstNonOrdered(values):
    ...
    while (not Compare(PoppedNode.state)) and (not emptyFlag):
        if (index == 0):
            PoppedNode = q.get()[2]
        for square in squares:
            if len(PoppedNode.state[square])>1:
                s=square
                break
...
################################################################################
    ...
    iniVars()
    if not iterativeDeepening(obtainInput(sudoku)):
        print('Sudoku is not solvable')
        exit()
    displayInitialBoard(sudoku)
    iniVars()
    displayBoard(iterativeDeepening(obtainInput(sudoku)))
